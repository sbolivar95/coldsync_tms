...
export default function App() {
  ...
  const [breadcrumbsState, setBreadcrumbsState] = useState<Record<string, BreadcrumbItem[]>>({});
  const [resetTrigger, setResetTrigger] = useState(0);
  const [transportistasActiveTab, setTransportistasActiveTab] = useState("todos"); // Estado para el tab activo de Transportistas
    ...
    switch (activeView) {
        ...
        return (
          <Dispatch 
            ...
            onBreadcrumbChange={(breadcrumbs) => {
              setBreadcrumbsState({ ...breadcrumbsState, dispatch: breadcrumbs });
            }}
      ...
      case "carriers":
        return (
          <CarriersWrapper 
            onBreadcrumbChange={(breadcrumbs) => {
              setBreadcrumbsState({ ...breadcrumbsState, carriers: breadcrumbs });
            }}
            resetTrigger={resetTrigger}
      ...
      case "locations":
        return (
          <LocationsWrapper 
            ref={ubicacionesRef}
            onBreadcrumbChange={(breadcrumbs) => {
              setBreadcrumbsState({ ...breadcrumbsState, locations: breadcrumbs });
            }}
            resetTrigger={resetTrigger}
          />
        );
      ...
      case "alerts":
        return <Alerts />;
      case "settings":
        return <Settings onTabChange={setSettingsActiveTab} ref={settingsRef} />;
      case "profile":
        return <Profile />;
      default:
        return <Dashboard />;
  ...
  const config = {
    ...
    createLabel,
    breadcrumbs: dynamicBreadcrumbs.length > 0 ? dynamicBreadcrumbs : viewConfig[activeView].breadcrumbs
  };
