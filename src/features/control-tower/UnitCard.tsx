...
interface Unit {
  ...
  unit: string;
  trailer: string;
  trailerEsHibrido?: boolean; // ✅ Propiedad para remolques híbridos
  driver: string;
  location: string;
  status: string;
  speed: string;
...
export function UnitCard({ unit, isSelected, onClick }: UnitCardProps) {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full h-32 px-4 py-2.5 border-b border-gray-100 hover:bg-gray-50 transition-colors text-left flex flex-col justify-between ${
      ...
      <div className="flex items-start justify-between mb-1.5">
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center gap-1.5">
            <Circle
              className={`w-2 h-2 flex-shrink-0 ${getStatusDotColor(unit.status, unit.hasActiveTrip)}`}
            />
      ...
      <div className="text-xs text-gray-500 pt-1.5 border-t border-gray-100 flex items-center justify-between gap-2">
        {unit.reeferMode && unit.reeferSetpoint ? (
          <>
            ...
            {unit.reeferError && (
              <div
                className={`flex items-center gap-1 ${getErrorSeverityColor(unit.reeferError.severity)} font-semibold flex-shrink-0`}
              >
