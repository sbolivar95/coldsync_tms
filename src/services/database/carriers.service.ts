...
export const carriersService = {
  ...
  /**
   * Get carrier type enum values from database
   */
  async getCarrierTypeOptions(): Promise<Array<{ value: string; label: string }>> {
    const { data, error } = await supabase.rpc('get_carrier_type_options')
  ...
  /**
   * Get currency options from database enum
   */
  async getCurrencyOptions(): Promise<Array<{ value: string; label: string }>> {
    const { data, error } = await supabase.rpc('get_currency_code_options')
